...
### Changed
- Recent Events panel (toggled with `[e]` in pane/floating UI) now subscribes to the daemon's `recent_event` broadcasts instead of the terse `event` classifications, so each row shows the actual command text plus exit code (for `cmd` events), the agent source and sub-kind (for `agent_event` entries), or the pattern-matched output line, rather than opaque labels like `generic:error`. Color/icon selection is driven by the event record's kind and exit code so failed commands render red with `x` and successful ones green with `.`.
...
### Fixed
- `resolveSelfInvocation()` (child-process re-entry for `setup`, `ui`, auto-spawn daemon, `chat`, etc.) no longer prefers a pre-built `dist/devbuddy.js` when the parent process is running TypeScript source via `tsx`. Historical behavior was a perf shortcut so children didn't pay the `tsx` transform cost, but it silently ran a stale `dist/` whenever the user edited source without rebuilding. In a recent iteration this caused the floating buddy window (spawned by `npm run dev -- setup`) to error with `error: unknown option '--primary'` because the running parent had the new flag but the on-disk `dist/` did not. The new contract: when the parent is TypeScript, all children also re-run TypeScript via `npx tsx <script>`. Production installs (`node dist/devbuddy.js`, global npm install) are unaffected because they already run `.js`. The `test/unit/core/self-spawn.test.ts` test that locked in the old preference was updated to assert the new contract.
- Floating buddy window on Windows Terminal (`wt.exe`) failed to launch with `0x80070002 (file not found)` when `--size 40,14` was passed to the `nt` (new-tab) subcommand. Root cause: `--size` is only a valid flag on the top-level `wt.exe` invocation in recent Terminal builds; `nt` does not recognize it. wt.exe silently stripped the unknown `--size` flag and treated `40,14` as the executable name to launch, producing the error. `src/ui/floating.ts` no longer passes `--size` to `nt`; the window size is taken from the user's Windows Terminal profile settings instead.
- Floating buddy window previously left the daemon running as an orphaned background process after the user closed it. The daemon now shuts itself down when the "primary" UI client disconnects. Implemented via a new optional `primary: boolean` field on the `subscribe` IPC message: the `DaemonServer` records an `isPrimary` flag on each `ClientConnection`, fires a new `onDisconnect` callback on socket close, and the orchestrator's `handleClientDisconnect` triggers `stop()` when the disconnecting client was primary. `DaemonClient` gained a `setPrimary(true)` helper that re-asserts the flag across auto-reconnects. `launchUI({ primary: true })` and the new `devbuddy ui --primary` flag expose this; `launchFloating` sets it automatically so closing the floating window cleans up the daemon. The in-process `devbuddy start` pane does not use `--primary` so its explicit daemon start is preserved.
- `devbuddy setup` daemon start timeout bumped from 5 s to 10 s. Cold Node starts (tsx transform, module resolution, registry load) occasionally exceeded 5 s on fresh checkouts, causing a spurious "Daemon failed to start" message even when the daemon came up moments later. The new timeout matches `devbuddy chat`'s existing 10 s budget.
- Floating buddy window failed to open with `0x80070002 (file not found)` on Windows when the node executable path contained spaces (e.g. `C:\Program Files\nodejs\node.exe`). Root cause: passing the invocation through `wt.exe` and `powershell -Command` required multiple overlapping quoting passes (wt.exe command-line parser, Node's CreateProcess quoting, PowerShell's `-Command` parser), and any of them dropped or mis-nested the quotes around the path. `src/ui/floating.ts` now builds a PowerShell script string using single-quoted literals and the call operator `&`, then base64-encodes it as UTF-16LE and passes it via `powershell -EncodedCommand <base64>`. The encoded form contains no spaces, quotes, or metacharacters, so it survives every layer of command-line parsing unchanged. Detection of `pwsh` vs `powershell.exe` via `where.exe` is retained. Both the Windows Terminal (`wt.exe`) and direct PowerShell fallback paths use the encoded form; the legacy `cmd.exe /k` last-resort fallback uses simple double-quote quoting.
- Floating window launched `devbuddy ui --mode pane` as a literal string, which only works for globally installed builds. Now uses `resolveSelfInvocation()` (same as the daemon self-spawn) to reconstruct the correct entry point — `node dist/devbuddy.js` when a build exists, or `npx tsx bin/devbuddy.ts` in dev mode — so the floating window works with `npm run dev` without a prior build.
